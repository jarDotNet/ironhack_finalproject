...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const handleDrop = async (dropResult) => {
      ...
      if (addedIndex !== null) {
        const original_task = { ...payload };

        const position = cardPosition(state.tasks, addedIndex);
        payload.current_state = props.current_sate;
        payload.pos = position;

        tasksStore.updateStoredTask(payload);
        const error = await tasksStore.markAs(
          props.current_sate,
          payload.id,
          position
        );
        if (error) {
          tasksStore.updateStoredTask(original_task);
        }
      }
    };
